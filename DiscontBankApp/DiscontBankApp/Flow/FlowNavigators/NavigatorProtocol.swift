...
//    associatedtype FlowController//each FC will set itself as the associatedtype when calling back to the higher level FC
...
//    func didFinishFlow(for destination : Destination)//when a specific flow had finished (the last screen was shown), we need to let a higher order FC know about it.
}
